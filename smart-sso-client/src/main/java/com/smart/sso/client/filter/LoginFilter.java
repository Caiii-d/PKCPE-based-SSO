...
import com.smart.sso.client.util.PkceUtils;
...
public class LoginFilter extends ClientFilter {
	...
	@Override
	public boolean isAccessAllowed(HttpServletRequest request, HttpServletResponse response) throws IOException, NoSuchAlgorithmException {
	...
	private void getAccessToken(String code, HttpServletRequest request) {
		System.out.println("vvvvvvvvvvvvvv"+(String) request.getSession().getAttribute(Oauth2Constant.CODE_VERIFIER));
		Result<RpcAccessToken> result = Oauth2Utils.getAccessToken(getServerUrl(), getAppId(),
				getAppSecret(), code, request.getSession().getAttribute(Oauth2Constant.CODE_VERIFIER));
		if (!result.isSuccess()) {
	 ...
	 */
	private void redirectLogin(HttpServletRequest request, HttpServletResponse response) throws IOException, NoSuchAlgorithmException {
		if (isAjaxRequest(request)) {
			responseJson(response, SsoConstant.NO_LOGIN, "未登录或已超时");
		}
		else {
			String codeVerifier = PkceUtils.generateCodeVerifier();
			//System.out.println("veriririririri"+codeVerifier);
			request.getSession().setAttribute(Oauth2Constant.CODE_VERIFIER, codeVerifier);
			...
			String loginUrl = new StringBuilder().append(getServerUrl()).append(SsoConstant.LOGIN_URL).append("?")
					...
					.append(URLEncoder.encode(getCurrentUrl(request), "utf-8")).append("&")
					.append(Oauth2Constant.CODE_CHALLENGE).append("=")
					.append(codeChallenge).append("&")
					.append(Oauth2Constant.CODE_CHALLENGE_METHOD).append("=")
					.append("S256")
					.toString();
	...
	protected boolean isAjaxRequest(HttpServletRequest request) {
        //获取请求头中的X-Requested-With
        String requestedWith = request.getHeader("X-Requested-With");
        //判断X-Requested-With是否为XMLHttpRequest，如果是则返回true，否则返回false
